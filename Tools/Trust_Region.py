...
from Tools.Newton_CG import CG_Steihaug
...
class TrustRegion:
    ...
    @staticmethod
    def Newton_CG_Method(f, grad, hess, x0, delta_hat=1.0, delta_0=0.2, eta=0.15):
        x = x0
        delta = delta_0
        ...
        for k in range(100):
            ...
            # 1. Find pk
            p, status = CG_Steihaug(g, B, delta)
            print(f"Iter {k}: {status}")
            # 2. Calculate rho
            actual_red = f(x) - f(x + p)
            predicted_red = -(np.dot(g, p) + 0.5 * np.dot(p, np.dot(B, p)))
